...
def SearchALL(pageNum):
    ...
    if(isExists): #Have recipe in database
        sql = "SELECT m.*, p.*, u.fName, u.lName, IF(ISNULL(f.favoriteStatus),FALSE ,TRUE) AS favoriteStatus, f.favoriteid FROM menu m LEFT JOIN public p ON m.menuid = p.menuid LEFT JOIN userinfo u ON m.createruid = u.uid LEFT JOIN favorite f ON f.menuid = m.menuid WHERE p.publicstatus = True ORDER BY avgVote DESC,menuName ASC LIMIT %s, %s"
        val = (pageNum - 100, pageNum)
        mycursor.execute(sql,val)
        result = mycursor.fetchall()
...
def SearchByName(menuName,pageNum):
    ...
    if(isExists): #Have recipe in database
        sql = "SELECT m.*,p.*, u.fName, u.lName, IF(ISNULL(f.favoriteStatus),FALSE ,TRUE) AS favoriteStatus, f.favoriteid FROM menu m LEFT JOIN public p ON m.menuid = p.menuid LEFT JOIN favorite f ON f.menuid = m.menuid LEFT JOIN userinfo u ON m.createruid = u.uid WHERE p.publicstatus = TRUE AND m.menuName LIKE %s ORDER BY avgVote DESC, m.menuName ASC LIMIT %s, %s"
        val = (menuName, pageNum - 100, pageNum)
        mycursor.execute(sql, val)
        result = mycursor.fetchall()
...
def SearchBycategory(categoryid, pageNum):
    ...
    if(isExists): #Have recipe in database
        sql = "SELECT m.*, p.*, c.categoryName, u.fName, u.lName, IF(ISNULL(f.favoriteStatus),FALSE ,TRUE) AS favoriteStatus, f.favoriteid FROM menu m LEFT JOIN public p ON m.menuid = p.menuid LEFT JOIN favorite f ON f.menuid = m.menuid LEFT JOIN category c ON c.categoryid = m.categoryid LEFT JOIN userinfo u ON m.createruid = u.uid WHERE p.publicstatus = TRUE AND c.categoryid = %s ORDER BY avgVote DESC, m.menuName ASC LIMIT %s, %s"
        val = (categoryid, pageNum-100, pageNum)
        mycursor.execute(sql,val)
        result = mycursor.fetchall()
...
def SearchBycreater(uid, pageNum):
    ...
    if(isExists): #Have recipe in database
        sql = "SELECT m.*,p.*, u.fName, u.lName, IF(ISNULL(f.favoriteStatus),FALSE ,TRUE) AS favoriteStatus, f.favoriteid FROM menu m LEFT JOIN public p ON m.menuid = p.menuid LEFT JOIN favorite f ON f.menuid = m.menuid LEFT JOIN userinfo u ON m.createruid = u.uid WHERE p.publicstatus = TRUE AND m.createruid = %s ORDER BY f.favoriteDate ASC, m.menuName ASC LIMIT %s, %s"
        val = (uid, pageNum - 100, pageNum)
        mycursor.execute(sql,val)
        result = mycursor.fetchall()
